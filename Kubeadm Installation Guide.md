...
2.	Locate Security Groups: 
    o	In the left menu under Network & Security, click on Security Groups. 
3.	Create a New Security Group: 
    o	Click on Create Security Group. 
    o	Provide the following details: 
    o	Name: (e.g., Kubernetes-Cluster-SG) 
    o	Description: A brief description for the security group (mandatory)
    o	VPC: Select the appropriate VPC for your instances (default is acceptable)
4.	Add Rules to the Security Group:
    o	Allow SSH Traffic (Port 22):
        a.	Type: SSH 
        b.	Port Range: 22 
        c.	Source: 0.0.0.0/0 (Anywhere) or your specific IP 
    o	Allow Kubernetes API Traffic (Port 6443):
        a.	Type: Custom TCP 
        b.	Port Range: 6443 
        c.	Source: 0.0.0.0/0 (Anywhere) or specific IP ranges 
5.	Save the Rules:
    o	Click on Create Security Group to save the settings.

Step 2: Select the Security Group While Creating Instances 
o	When launching EC2 instances: 
    o	Under Configure Security Group, select the existing security group (Kubernetes-Cluster-SG) 
...
**Execute on Both "Master" & "Worker" Nodes **
   ...
   4.    Install Containerd:
    ...
    containerd config default | sed -e 's/SystemdCgroup = false/SystemdCgroup = true/' -e 's/sandbox_image = "registry.k8s.io/pause:3.6"/sandbox_image = "registry.k8s.io/pause:3.9"/' | 
    xsudo tee /etc/containerd/config.toml
...
2.    Paste the join command you got from the master node and append --v=5 at the end:
    ...
    sudo kubeadm join :6443 --token --discovery-token-ca-cert-hash sha256: --cri-socket "unix:///run/containerd/containerd.sock" --v=5 
